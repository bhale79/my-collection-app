...
function buildPrefsPage() {
  ...
  el.innerHTML = `
    ...
    <div class="pref-section" id="admin-health-section" style="display:none">
      ...
      <div class="pref-row" style="margin-top:0.25rem">
        <div class="pref-row-label"><strong>Backfill Era Tags</strong><span>Tag collection items with correct era (PW/MPC/Mod)</span></div>
        <button class="pref-btn" id="backfill-era-btn" onclick="_runBackfillEra()">Run Backfill</button>
      </div>
      <div id="backfill-era-output" style="display:none;margin-top:0.75rem;background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:0.85rem 1rem;font-size:0.75rem;font-family:var(--font-mono);line-height:1.8;max-height:320px;overflow-y:auto"></div>
      </div>
    </div>
...
// ── Backfill Era Tags ──────────────────────────────────────────
// Reads item numbers from all era master tabs, matches them to
// personal collection items, and writes the correct era to col X.
async function _runBackfillEra() {
  var btn = document.getElementById('backfill-era-btn');
  var out = document.getElementById('backfill-era-output');
  if (!out) return;
  out.style.display = 'block';
  out.innerHTML = '<span style="color:var(--text-dim)">Loading master data from all eras…</span>';
  if (btn) { btn.disabled = true; btn.textContent = 'Running…'; }
  ...
  try {
    ...
    out.innerHTML = '<span style="color:var(--text-dim)">Reading ' + ranges.length + ' master tabs…</span>';

    // 2. Batch-read all master tab item numbers
    var batchRes = await sheetsBatchGet(MASTER_SHEET_ID, ranges);
    var vrs = batchRes.valueRanges || [];

    // 3. Build lookup: normalizedItemNum -> era (first match wins)
    var eraLookup = {};
    for (var r = 0; r < vrs.length; r++) {
      var rows = vrs[r].values || [];
      var era = rangeEra[r];
      for (var j = 0; j < rows.length; j++) {
        var num = normalizeItemNum(rows[j][0] || '');
        if (num && !eraLookup[num]) eraLookup[num] = era;
      }
    }
    var lookupSize = Object.keys(eraLookup).length;
    lines.push('Loaded ' + lookupSize + ' master item numbers across ' + Object.keys(ERA_TABS).length + ' eras');

    // 4. Find personal items needing era update
    var updates = []; // { row, era, itemNum }
    var alreadyTagged = 0;
    Object.values(state.personalData).forEach(function(pd) {
      if (!pd.row) return;
      if (pd.era && pd.era.trim()) { alreadyTagged++; return; }
      var num = normalizeItemNum(pd.itemNum);
      var matchedEra = eraLookup[num] || 'pw'; // default to pw for unmatched
      updates.push({ row: pd.row, era: matchedEra, itemNum: pd.itemNum });
    });

    lines.push(alreadyTagged + ' items already have era tags');
    lines.push(updates.length + ' items need era tags');

    if (updates.length === 0) {
      lines.push('');
      lines.push('✅ All items already have era tags — nothing to do!');
    } else {
      // 5. Write era tags using values:batchUpdate for efficiency
      out.innerHTML = lines.join('<br>') + '<br><span style="color:var(--text-dim)">Writing era tags…</span>';

      // Batch in groups of 500 to stay within API limits
      var BATCH = 500;
      for (var b = 0; b < updates.length; b += BATCH) {
        var chunk = updates.slice(b, b + BATCH);
        var data = chunk.map(function(u) {
          return { range: 'My Collection!X' + u.row, values: [[u.era]] };
        });
        ...
        if (!res.ok) {
          ...
          throw new Error('Batch update failed (' + res.status + '): ' + errText.slice(0, 200));
        }

        lines.push('Wrote batch ' + (Math.floor(b/BATCH)+1) + ': ' + chunk.length + ' items');
        out.innerHTML = lines.join('<br>');
      }

      // 6. Count by era for summary
      var eraCounts = {};
      updates.forEach(function(u) {
        eraCounts[u.era] = (eraCounts[u.era] || 0) + 1;
      });

      // 7. Update in-memory state
      updates.forEach(function(u) {
        Object.values(state.personalData).forEach(function(pd) {
          if (pd.row === u.row) pd.era = u.era;
        });
      });
      ...
      Object.keys(ERAS).forEach(function(ek) {
        if (eraCounts[ek]) {
          lines.push('✅ ' + ERAS[ek].label + ': ' + eraCounts[ek] + ' items tagged');
        }
      });
      lines.push('');
      lines.push('Done! ' + updates.length + ' items updated. Dashboard will refresh.');
      fixed = updates.length;

      // Refresh dashboard to show new era counts
      buildDashboard();
    }

  } catch(e) {
    lines.push('❌ Error: ' + e.message);
    console.error('Backfill era error:', e);
  }
