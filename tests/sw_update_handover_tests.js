...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1771' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1780', 'mca-v1781']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1780 deleted, mca-v1781 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1780', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1771'\)/.test(ix));
...
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1781';/.test(swSrc));
ok('index.html stamps every asset at 1771 and none at 1770',
   (ix.match(/\?v=1771/g) || []).length === 79 && !/\?v=1770/.test(ix));
