...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1761' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1770', 'mca-v1771']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1770 deleted, mca-v1771 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1770', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1761'\)/.test(ix));
...
ok('APP_VERSION v0.9.1761', /const APP_VERSION = 'v0\.9\.1761';/.test(cfg));
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1771';/.test(swSrc));
ok('index.html stamps every asset at 1761 and none at 1760',
   (ix.match(/\?v=1761/g) || []).length === 79 && !/\?v=1760/.test(ix));
