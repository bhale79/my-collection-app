...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1779' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1788', 'mca-v1789']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1788 deleted, mca-v1789 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1788', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1779'\)/.test(ix));
...
ok('APP_VERSION v0.9.1779', /const APP_VERSION = 'v0\.9\.1779';/.test(cfg));
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1789';/.test(swSrc));
ok('index.html stamps every asset at 1779 and none at 1778',
   (ix.match(/\?v=1779/g) || []).length === 79 && !/\?v=1778/.test(ix));
