...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1763' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1772', 'mca-v1773']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1772 deleted, mca-v1773 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1772', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1763'\)/.test(ix));
...
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1773';/.test(swSrc));
