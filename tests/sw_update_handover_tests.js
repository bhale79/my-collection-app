...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1762' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1771', 'mca-v1772']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1771 deleted, mca-v1772 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1771', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1762'\)/.test(ix));
...
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1772';/.test(swSrc));
