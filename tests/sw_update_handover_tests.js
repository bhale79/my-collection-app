...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1768' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1777', 'mca-v1778']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1777 deleted, mca-v1778 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1777', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1768'\)/.test(ix));
...
ok('APP_VERSION v0.9.1768', /const APP_VERSION = 'v0\.9\.1768';/.test(cfg));
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1778';/.test(swSrc));
ok('index.html stamps every asset at 1768 and none at 1767',
   (ix.match(/\?v=1768/g) || []).length === 79 && !/\?v=1767/.test(ix));
