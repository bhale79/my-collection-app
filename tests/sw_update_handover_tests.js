...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1764' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1773', 'mca-v1774']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1773 deleted, mca-v1774 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1773', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1764'\)/.test(ix));
...
ok('APP_VERSION v0.9.1764', /const APP_VERSION = 'v0\.9\.1764';/.test(cfg));
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1774';/.test(swSrc));
ok('index.html stamps every asset at 1764 and none at 1763',
   (ix.match(/\?v=1764/g) || []).length === 79 && !/\?v=1763/.test(ix));
