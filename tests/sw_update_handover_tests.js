...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1770' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1779', 'mca-v1780']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1779 deleted, mca-v1780 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1779', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1770'\)/.test(ix));
...
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1780';/.test(swSrc));
ok('index.html stamps every asset at 1770 and none at 1769',
   (ix.match(/\?v=1770/g) || []).length === 79 && !/\?v=1769/.test(ix));
