...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1775' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1784', 'mca-v1785']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1784 deleted, mca-v1785 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1784', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1775'\)/.test(ix));
...
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1785';/.test(swSrc));
