...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1774' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1783', 'mca-v1784']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1783 deleted, mca-v1784 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1783', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1774'\)/.test(ix));
...
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1784';/.test(swSrc));
