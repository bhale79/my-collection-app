...
function runSW() {
  ...
  const self_ = {
    location: { href: 'https://therailroster.com/app/sw.js?v=1782' },
  ...
  const caches_ = {
    ...
    keys: () => Promise.resolve(['mca-v1791', 'mca-v1792']),
...
ok('activate deletes every cache that is not this version, and keeps this one',
   actWaits.length === 1);
actWaits[0].then(() => {
  ok('…verified: mca-v1791 deleted, mca-v1792 kept',
     sw.calls.deleted.length === 1 && sw.calls.deleted[0] === 'mca-v1791', JSON.stringify(sw.calls.deleted));
  rest();
});
...
ok('the worker URL is stamped with this version', /register\('\.\/sw\.js\?v=1782'\)/.test(ix));
...
ok('CACHE_NAME is the version + 10', /const CACHE_NAME = 'mca-v1792';/.test(swSrc));
ok('index.html stamps every asset at 1782 and none at 1781',
   (ix.match(/\?v=1782/g) || []).length === 79 && !/\?v=1781/.test(ix));
