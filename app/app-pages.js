    ...
    cardsEl.innerHTML = shownEntries.map(w => {
      ...
      return `<div id="share-card-${_wShareKey}" style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:0.85rem 1rem${_wSelected ? ';outline:2px solid #2ecc71' : ''}">
        ...
        ${!_wInShare ? `<div style="display:grid;grid-template-columns:1fr 1fr;gap:0.4rem;margin-top:0.6rem">
          <button onclick="event.stopPropagation();moveWantToCollection('${w.itemNum}','${escVar}')" style="min-width:0;padding:0.45rem 0.3rem;border-radius:7px;font-size:0.75rem;cursor:pointer;border:1.5px solid #2ecc71;background:var(--bg-card);background:color-mix(in srgb, rgb(46,204,113) 12%, var(--bg-card));color:#2ecc71;font-family:var(--font-body);font-weight:600">+ Collection</button>
    ...
    tbody.innerHTML = shownEntries.map((w, idx) => {
      ...
      return `<tr id="share-card-${_wDShareKey}" ${_wDInShare ? 'onclick="toggleShareItem(\'' + _wDShareKey + '\')"' : ''} style="cursor:${_wDInShare ? 'pointer' : 'default'}${_wDSelected ? ';outline:2px solid #2ecc71;background:rgba(46,204,113,0.06)' : ''}">
        ...
        <td style="white-space:nowrap">
          ${!_wDInShare ? `<div style="display:grid;grid-template-columns:1fr 1fr;gap:0.3rem;min-width:190px"><button onclick="moveWantToCollection('${w.itemNum}','${(w.variation||'').replace(/'/g,"\\'")}')" style="padding:0.2rem 0.45rem;border-radius:5px;font-size:0.7rem;cursor:pointer;border:1px solid #2ecc71;background:var(--bg-card);background:color-mix(in srgb, rgb(46,204,113) 12%, var(--bg-card));color:#2ecc71;font-family:var(--font-body)" title="Add to My Collection">+ Collection</button>
    ...
    cardsEl.innerHTML = entries.map(u => {
      ...
      return `<div ${_wuInShare ? `id="share-card-${_wuMKey}" onclick="toggleShareItem('${_wuMKey}')"` : `onclick="_wantViewDetail('${u.itemNum}','${escVar}')"`} style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:0.85rem 1rem;cursor:pointer${_wuMSel ? ';outline:2px solid #2ecc71;background:rgba(46,204,113,0.08)' : ''}">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.4rem;margin-top:0.6rem">
          ...
          ${_isWant
            ? `<button onclick="event.stopPropagation();moveWantToCollection('${u.itemNum}','${escVar}')" style="min-width:0;padding:0.45rem 0.3rem;border-radius:7px;font-size:0.72rem;cursor:pointer;border:1.5px solid #2ecc71;background:var(--bg-card);background:color-mix(in srgb, rgb(46,204,113) 12%, var(--bg-card));color:#2ecc71;font-family:var(--font-body);font-weight:600">+ Collection</button>`
    ...
    tbody.innerHTML = entries.map((u, idx) => {
      ...
      return `<tr${_wuTrAttrs}>
        ...
        <td style="white-space:normal">
          ...
          ${_isWant
            ? `<button onclick="event.stopPropagation();moveWantToCollection('${u.itemNum}','${escVar}')" style="padding:0.2rem 0.45rem;border-radius:5px;font-size:0.7rem;cursor:pointer;border:1px solid #2ecc71;background:var(--bg-card);background:color-mix(in srgb, rgb(46,204,113) 12%, var(--bg-card));color:#2ecc71;font-family:var(--font-body);font-weight:600;margin-right:0.2rem">+ Collection</button>`
