    ...
    cardsEl.innerHTML = shownEntries.map(w => {
      ...
      return `<div id="share-card-${_wShareKey}" style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:0.85rem 1rem${_wSelected ? ';outline:2px solid #2ecc71' : ''}">
        ...
        ${!_wInShare ? `<div style="display:flex;gap:0.35rem;margin-top:0.6rem;flex-wrap:wrap">
          ...
          <button onclick="event.stopPropagation();removeWantItem('${w.itemNum}','${escVar}',${w.row})" style="flex:0 0 auto;padding:0.4rem 0.6rem;border-radius:7px;font-size:0.75rem;cursor:pointer;border:1.5px solid var(--border);background:var(--surface2);color:#f05008;font-family:var(--font-body)">Remove</button>
        </div>` : ''}
      </div>`;
    }).join('');
  } else {
    ...
    cardsEl.innerHTML = entries.map(u => {
      ...
      return `<div onclick="_wantViewDetail('${u.itemNum}','${escVar}')" style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:0.85rem 1rem;cursor:pointer">
        ...
        <div style="display:flex;gap:0.35rem;margin-top:0.6rem;flex-wrap:wrap">
          ...
          ${_isWant
            ? `<button onclick="event.stopPropagation();removeWantItem('${u.itemNum}','${escVar}',${u.row})" style="flex:0 0 auto;padding:0.4rem 0.6rem;border-radius:7px;font-size:0.72rem;cursor:pointer;border:1.5px solid var(--border);background:var(--surface2);color:#f05008;font-family:var(--font-body)">Remove</button>`
            : `<button onclick="event.stopPropagation();removeUpgradeItem('${_ugEntryKey(u)}')" style="flex:0 0 auto;padding:0.4rem 0.6rem;border-radius:7px;font-size:0.72rem;cursor:pointer;border:1.5px solid var(--border);background:var(--surface2);color:#f05008;font-family:var(--font-body)">Remove</button>`}
        </div>
      </div>`;
    }).join('');
  } else {
