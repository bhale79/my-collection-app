...
(function() {
  ...
  // v0.9.783 (contacts hook): decode a QR code from a still canvas — the
  // Contacts card scanner checks for a QR (often a vCard) BEFORE any reading.
  window._decodeQrText = async function (canvas) {
    try {
      if ('BarcodeDetector' in window) {
        try {
          var qDet = new window.BarcodeDetector({ formats: ['qr_code'] });
          var qGot = await qDet.detect(canvas);
          if (qGot && qGot.length && qGot[0].rawValue) return String(qGot[0].rawValue);
        } catch (eN) {}
      }
      var qMod = await _loadZXing();
      var qCtx = canvas.getContext('2d');
      var qImg = qCtx.getImageData(0, 0, canvas.width, canvas.height);
      var qRes = await qMod.readBarcodesFromImageData(qImg, { tryHarder: true, formats: ['QRCode'], maxNumberOfSymbols: 1 });
      if (qRes && qRes.length && qRes[0].text) return String(qRes[0].text);
    } catch (eQ) {}
    return null;
  };
    ...
    try {
      ...
      if (!meta.itemNum) {
        ...
        if (meta.cabNum && meta.manufacturer) {
          ...
          if (cabOk.length) {
            var cm = cabOk[0];
            return { handled: true, itemNum: cm.itemNum, variation: cm.variation || '', masterItem: cm, manufacturer: meta.manufacturer, roadName: (cm.roadName || ''), description: (cm.description || ''), eraTag: (typeof _eraLabel === 'function') ? _eraLabel(cm._era) : '', verifiedNote: '🔍 Box number ' + cabRaw + ' read from the photo', verifiedBy: 'ai', statusMessage: 'Found ' + cm.itemNum + ' — ' + (cm.description || '').substring(0, 40) };
          }
        }
  ...
  // v0.9.738 (Brad): "add some drop downs to help make a stronger search if a
  // user is just entering a number". v0.9.739: era collapsed to Prewar/Postwar/
  // Modern (collector language, not our 12 catalog tabs), + Scale, and all
  // three REMEMBER the last pick (localStorage) so a show-floor user sets
  // them once.
  function _biQuickFilters() {
    var sv = function (k) { try { return localStorage.getItem('lv_rsq_' + k) || ''; } catch (e) { return ''; } };
    var vM = sv('mfr'), vE = sv('era'), vS = sv('scale');
    var mfrs = [], seen = {};
    try {
