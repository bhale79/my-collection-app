...
function renderWizardStep() {
  ...
  if (s.type === 'manualManufacturer') {
        ...
        <div style="padding-top:0.5rem">
          ...
          <div style="display:flex;flex-direction:column;gap:0.5rem" id="var-cards">
            ${variations.map(v => {
              ...
              //
              // v0.9.1335 — THE SAME BUTTON, A THIRD WAY (Brad's 3472, the
              // inbox ADD flow). Event log from the live app: mousedown hits
              // the anchor → the anchor takes FOCUS → Chrome scrolls
              // #wizard-body 79px to "reveal" the focused link (the inbox
              // flow's banner + photo panel create the geometry that
              // triggers it) → mouseup lands on the description that slid
              // under the cursor → the click retargets to the CARD → picks
              // the variation and auto-advances. stopPropagation never runs
              // because the click never belongs to the anchor. The guard is
              // onmousedown=preventDefault: no focus on press, no scroll
              // nudge, the click completes ON the link. Tab-key focus is
              // untouched. Do NOT remove either handler; they stop
              // DIFFERENT failure modes of the same button.
              const cottLink = v.refLink ? `<a href="${(typeof window.cottAnchorUrl==='function') ? window.cottAnchorUrl(v.refLink, itemNum, window.cottRowWords ? window.cottRowWords(v) : '', v.variation || '') : v.refLink}" target="_blank" rel="noopener" onclick="event.stopPropagation()" onmousedown="event.preventDefault()" style="display:inline-flex;align-items:center;gap:0.3rem;font-size:0.78rem;color:var(--accent2);text-decoration:none;padding:0.42rem 0.7rem;border:1px solid rgba(201,146,42,0.45);border-radius:6px;background:var(--bg-card);background:color-mix(in srgb, rgb(201,146,42) 12%, var(--bg-card));flex-shrink:0;white-space:nowrap;font-weight:600;position:relative;z-index:1;min-height:34px;box-sizing:border-box">${_refShort}</a>` : '';
              return `
    ...
    renderWizardStep();
    return;
    // v0.9.1708 (silent-control audit B5): ~290 lines of Quick-Entry Step 1
    // renderers used to sit here, AFTER the return above — unreachable since the
    // day the step was auto-skipped ("QE renderer needs rebuild — code was lost").
    // Their three buttons could only ever look for panels that no code built.
    // (_selectTender still checks for #qe1-tender-label and falls through to
    // its Step-3 re-render when it is absent, which is now always.)
    ...
    _bcdHtml += '<div style="display:flex;justify-content:space-between;font-size:0.7rem;color:var(--text-dim);margin-top:-0.35rem;margin-bottom:0.75rem"><span>Poor</span><span>Excellent' + _wizCondHelp() + '</span></div>';
  ...
  // saves a representative year so the chip-filter classifier works.
    ...
    if (!_ingPreFilled && !window.IS_MOBILE_UA) {
      ...
      // v0.9.1360: given an id so the help guide (and any test) can point at
      // it. It had NO id and NO class — only a style attribute — so the only
      // way to find it was by its text, and a guide that finds its target by
...
// ── v0.9.1551b (Brad: the "?" was not on his screen) ────────────────────
// The condition slider is drawn by FIVE renderers in this file — quick
// entry, the set-entry panel, the box-condition panel, the photo-step embed,
// and the plain slider step. v0.9.1550 patched three of them, which is how a
// control ends up present on some screens and missing on the one you happen
// to open. Every renderer ends with the same "Poor … Excellent" scale line,
// so the link lives THERE, written once, and appears wherever a condition is
// asked for.
function _wizCondHelp() {
  return ' <button type="button" title="What do the numbers mean?"'
    + ' onclick="event.preventDefault();event.stopPropagation();if(window.rrgOpen)rrgOpen(\'grading\')"'
    + ' style="border:1px solid var(--border);background:none;color:var(--text-dim);border-radius:50%;'
    + 'width:14px;height:14px;line-height:1;font-size:0.62rem;cursor:pointer;padding:0;vertical-align:1px">?</button>';
}
if (typeof window !== 'undefined') window._wizCondHelp = _wizCondHelp;
