...
//     addFromBrowse
//   • (v0.9.1764: the item pop-up — _buildItemModal / openItem /
//     fillItemFromBoxRow / closeModal / closeModalOnOverlay / setStatus /
//     _saveItemWrites / saveItem — was DELETED. It could overwrite the wrong
//     copy's whole row, and it could not be opened. The item detail page is
//     now the one editing screen. See the note where it lived.)
...
// ── v0.9.1764: THE ITEM POP-UP IS GONE (about 540 lines) ─────────────
// Brad, after the copy-identity audit: "Delete it, send Edit to the detail
// page."
//
// What lived here: _buildItemModal, openItem, fillItemFromBoxRow, closeModal,
// closeModalOnOverlay, setStatus, _saveItemWrites and saveItem — a second,
// older editing screen for a catalog row.
//
// TWO reasons it went, and they were both accidents.
//
// 1. Its SAVE was the 6-24177 bug in a screen the v1761 sweep never looked at.
//    It found the owned copy with findPDKey — number and variation only — and
//    then REPLACED THAT WHOLE ROW. With two copies of an item that overwrites
//    the first one's condition, price paid, notes and photo link with whatever
//    the pop-up held. personalWriteRow's identity check could not have helped:
//    it confirms the row still IS that copy, and it was; the wrong copy had
//    been chosen upstream. Exactly the shape that cost Brad his 6-24177.
//
// 2. It could not actually be opened. Its only door was wizard.js's
//    _scEditItem, which passed a personal-data KEY where openItem wanted a
//    CATALOG INDEX; openItem had no guard, so it built its pop-up, put it on
//    screen, then threw on .itemNum of an undefined item. So the bug above was
//    unreachable only by luck, and the button was dead.
//
// Deleting beat repairing: the detail page already does this job and, since
// v1761, always knows which copy it is on. Keeping a second editing screen
// alive meant keeping a second place for "which copy?" to be answered wrongly.
// Same call as addPhotosFromCollection in v1761 — remove the gun, don't
// disarm it.
//
// Verified self-contained before cutting: state.currentItem, _saveItemBusy and
// _pendingWantPartner were read nowhere else; the DOM ids were referenced only
// here (index.html's mention was a comment); and modal-item-num / modal-title
// are CSS CLASS names shared by wizard.js and maintenance.js, so app.css keeps
// them. addFromBrowse (above) and addItemToWantList (below) are not part of
// this and stay.
